...
// Classification Information //
  ...
  .set('serviceCategoriesDesc', {mandatory: true, label: 'Service Categories', placeholder: 'Write Service Categories...', desc: 'Service Category identifies the broad category of the service being described in the Service Profile.  Possible values can be selected from the Controlled Vocabulary, and multiple categories can be selected.  We encourage Providers to suggest additional categories using the "Other" category, but only broad, generally useful suggestions will be accepted.'})
  .set('marketplaceLocationsDesc', {mandatory: true, label: 'Marketplace Locations', placeholder: 'Write Marketplace Locations ...', desc: 'Marketplace Location specifies the section(s) of the EOSC Marketplace where the service being described in the Service Profile should be displayed. Possible values can be selected from the Controlled Vocabulary, and multiple categories can be selected. The values are set by the Marketplace team.  Requests for new "locations" can be made through the Helpdesk and will be considered by the Marketplace team.'})
