...
export class TrainingListComponent implements OnInit {
  ...
  selectedTrainingResourceForDedup: TrainingResourceBundle = null;
  ...
  searchForDuplicates(bundle: TrainingResourceBundle) {
    if (!bundle?.id) {
      return;
    }
    this.errorMessage = '';
    ...
    this.selectedTrainingResourceForDedup = bundle;
    ...
    this.deduplicationService.findDuplicates('training_resource', bundle.id).subscribe({
      next: (similar) => {
        this.loadingMessage = '';
        ...
        if (list.length > 0) {
          ...
          setTimeout(() => {
            UIkit.modal('#trainingDuplicatesModal').show();
          }, 0);
        } else {
          UIkit.notification({
            message: 'No duplicate records found for this training resource.',
            status: 'success',
            pos: 'top-center',
            timeout: 4000
          });
  ...
  getDuplicateResource(item: SimilarResource): any {
    ...
    return item.result.trainingResource || item.result;
  }
  ...
  openDuplicateDetail(item: SimilarResource) {
    ...
    setTimeout(() => {
      UIkit.modal('#trainingDuplicateDetailModal').show();
    }, 0);
  }
